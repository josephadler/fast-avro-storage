      ...
      case DataType.BAG:
        {
          ...
          if (isNullableUnion(elementSchema))
            elementSchema = removeNullableUnion(elementSchema);
          switch (elementSchema.getType()) {
            case RECORD:
            case MAP:
            case ARRAY:
              innerResourceSchema = avroSchemaToResourceSchema(
                  elementSchema,namesInStack, alreadyDefinedSchemas);
              bagSchemaFields[0].setName(elementSchema.getName());
              break;
            case UNION:
              throw new IOException("Pig does not know how to translate avro schemas for complex unions");
            default:         
      ...
      case DataType.MAP:
        {
          ...
          if (isNullableUnion(mapAvroSchema))
            mapAvroSchema = removeNullableUnion(mapAvroSchema);
    ...
    if (rs.getFields() != null) {
      ...
      for (ResourceSchema.ResourceFieldSchema rfs : rs.getFields()) {
        ...
        fields.add(new Schema.Field(
              (rfs.getName() != null) ? rfs.getName() : recordName + "_" + i.toString(),
              fieldSchema, 
              rfs.getDescription().equals("autogenerated from Pig Field Schema") ? null : rfs.getDescription(),
              null)
            );
